...
외부 문서가 필요할 때 사용자는 아래 세 가지 방법 중 하나를 선택할 수 있다.
에이전트는 사용자가 어떤 방법을 선택했는지 파악한 뒤 그에 맞는 방식으로 문서를 읽는다.

### 방법 A — 파일 직접 첨부

사용자가 대화에 파일을 직접 첨부한 경우, 에이전트는 첨부된 내용을 그대로 읽는다.
별도 도구 호출이 필요 없고 가장 빠르다.

### 방법 B — URL 붙여넣기

사용자가 URL을 대화에 붙여넣은 경우, 에이전트는 `WebFetch` 도구로 해당 URL의 내용을 읽는다.
공개 웹 페이지나 공개 Notion 링크 등에 사용할 수 있다.
코드나 설정 파일에 URL을 직접 기재하지 않는다.

### 방법 C — Notion MCP 자동 탐색 (선택적)

사용자가 아무것도 제공하지 않고 Notion MCP가 연결된 경우, 에이전트가 스스로 문서를 찾는다.

1. `docs/references.json`에서 해당 문서의 `notion_search_query`를 확인한다.
2. `notion-search` 도구로 쿼리를 실행해 페이지를 찾는다.
3. 찾은 페이지 ID로 `notion-fetch` 도구를 실행해 내용을 읽는다.

### 폴백 규칙

- 세 방법 중 어느 것도 동작하지 않으면 로컬 파일만으로 작업을 계속한다.
- 참조한 문서 제목과 사용한 방법을 `_workspace/02_build_log.md`에 기록한다.
